...
<script>
   ...
   $(function(){
    ...
    var type = $('[name="seat_type"]').val();
    var fare = (type == 1) ? <?= $first_class_fare ?> : <?= $economy_fare ?>;
    var passengers = $('.reserve-item').length;
    var total_fare = fare * passengers;
    
    $('[name="fare_amount"]').val(total_fare.toFixed(2)); // Update fare field
}

$(function(){
    $('[name="seat_type"]').change(function(){
        update_total_fare(); // Recalculate when seat type changes
    });
    ...
    $('#add_item').click(function(){
        ...
        update_total_fare(); // Update fare when passenger is added
    });
    ...
    function remove_passenger($this) {
        if($('.reserve-item').length == 1) {
            alert("Can't Remove Passenger details. Reservation form requires at least 1 passenger.");
            return false;
        }
        $this.closest('.reserve-item').remove();
        update_total_fare(); // Update fare when passenger is removed
    }
});
